...
CREATE Procedure prj_0009_REP_001_Search_TelephoneContacts_return_All_Fields(
  ...
  FROM [d3_JPHSA_All_Users].[dbo].[T003_Telephone_Contacts]
			...
			WHERE
					(
							...
							OR
							(
								(upper(rtrim(ltrim([IT_Notes]))) like '%' +  UPPER(RTRIM(LTRIM(@SearchString))) + '%')
								AND
								(@use_IT_Notes = 1)
							)
					)		 
